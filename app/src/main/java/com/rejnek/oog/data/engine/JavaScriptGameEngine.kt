...
class JavaScriptGameEngine {
    ...
    private val gameElements = mutableMapOf<String, GameElement>()
    private var currentGameRef: Game? = null

    fun initialize(gameScript: String, gameContext: Game?): Result<Unit> {
        return try {
            ...
            // Store reference to the current game
            currentGameRef = gameContext

            // Add game API functions to JavaScript context
            addGameApiFunctions(gameContext)
    ...
    private fun addGameApiFunctions(gameContext: Game?) {
        scope?.let { scope ->
            ...
            val gameAPI = object : ScriptableObject() {
                ...
                fun jsFunction_show(elementName: String) {
                    gameContext?.elements?.find { it.name == elementName }?.let {
                        it.visible = true
                    }
                }
            }
    ...
    private fun convertJSObjectToGameElement(jsObj: Scriptable, name: String): GameElement {
        ...
        val gameType = ScriptableObject.getProperty(jsObj, "gameType")?.toString()
        ...
        Log.d("JavaScriptGameEngine", "Object properties - " +
                ...
                "Visible: ${gameElements.containsKey(objName)}")
    ...
    /**
     * Get the current game reference that might have been updated by JavaScript code
     */
    fun getCurrentGameRef(): Game? {
